...
async function removeFeedback() {
  if (!window.confirm('确定删除这条反馈及其全部图片附件吗？此操作无法撤销，也不会发布更新信息。')) return
  deleting.value = true
  errorMessage.value = ''
  try {
...
function openResolve() {
  resolveDialog.value?.open(feedback.value)
}

async function onResolved() {
  await router.replace({ name: 'admin-feedback', query: { resolved: '1' } })
}
    ...
    <section class="admin-page admin-detail-page" aria-labelledby="feedback-detail-title">
      ...
      <template v-else-if="feedback">
        <header class="admin-page-heading">
          <div><p class="utility-label">FEEDBACK / {{ feedback.id }}</p><h1 id="feedback-detail-title">{{ feedback.contact_name }} 的问题反馈</h1><p>提交时间：{{ formatDate(feedback.submitted_at) }}</p></div>
          <div class="admin-heading-actions"><button class="button button-primary" type="button" :disabled="deleting" @click="openResolve">问题已经解决</button><button class="button admin-danger-button" type="button" :disabled="deleting" @click="removeFeedback">{{ deleting ? '正在删除…' : '删除反馈' }}</button></div>
        </header>
        ...
        <section class="admin-data-panel admin-attachments"><div class="admin-section-heading"><h2>图片附件</h2><span>{{ feedback.image_attachments.length }} 张</span></div><p v-if="!feedback.image_attachments.length" class="admin-table-empty">此反馈没有图片附件。</p><ul v-else><li v-for="attachment in feedback.image_attachments" :key="attachment.id"><img :src="`/api/admin/feedback/${feedback.id}/attachments/${attachment.id}`" :alt="attachment.name" width="112" height="84" loading="lazy" /><div><strong>{{ attachment.name }}</strong><span>{{ attachment.contentType }} · {{ formatSize(attachment.size) }}</span></div><a class="button button-secondary" :href="`/api/admin/feedback/${feedback.id}/attachments/${attachment.id}?download=1`">下载</a></li></ul></section>
      </template>
    </section>
