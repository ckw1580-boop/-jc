...
<template>
  <UserAuthFrame :interlock-state="submitting ? '正在建立账号' : '等待注册'" step="STEP 00 / ENROLL">
    ...
    <form class="access-form" novalidate @submit.prevent="submit">
      ...
      <label class="field"><span>邮箱</span><input ref="emailInput" v-model="form.email" name="email" type="email" inputmode="email" autocomplete="email" spellcheck="false" maxlength="254" required :aria-invalid="Boolean(errors.email)" :aria-describedby="errors.email ? 'register-email-error' : undefined" @input="errors.email = ''" /><span v-if="errors.email" id="register-email-error" class="field-error">{{ errors.email }}</span></label>
      <label class="field"><span>密码</span><input ref="passwordInput" v-model="form.password" name="new-password" type="password" autocomplete="new-password" minlength="10" maxlength="128" required :aria-invalid="Boolean(errors.password)" :aria-describedby="errors.password ? 'register-password-error' : 'register-password-help'" @input="errors.password = ''; errors.confirmPassword = ''" /><small id="register-password-help">10–128 个字符，允许使用密码管理器粘贴</small><span v-if="errors.password" id="register-password-error" class="field-error">{{ errors.password }}</span></label>
